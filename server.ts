...
function logAxiosError(source: string, error: any, url?: string) {
  if (axios.isAxiosError(error)) {
    ...
    if (error.response?.status === 403) {
      console.error(`[${source}] 403 Forbidden - likely blocked by anti-bot measures`);
    }
  } else {
...
function normalizeJobUrl(url: string): string {
  ...
  try {
    ...
    // For Indeed, 'jk' is critical. For others, usually we want to strip everything.
...
async function startServer() {
  ...
  app.post("/api/scrape", scrapeLimiter, async (req, res) => {
    ...
    // Use a Map to deduplicate by normalized URL across ALL queries and sources
    ...
    try {
      for (const query of queries) {
        ...
        if (selectedSources.includes("LinkedIn")) {
          ...
          try {
            ...
            $('li, .base-search-card, .job-search-card').each((_, el) => {
              ...
              if (title && link) {
                const normUrl = normalizeJobUrl(link);
                if (!jobsMap.has(normUrl)) {
        ...
        if (selectedSources.includes("Indeed")) {
          const indeedUrl = `https://ch.indeed.com/jobs?q=${encodeURIComponent(query)}&l=Switzerland&sort=date`;
          try {
            // Jittered delay
            ...
            const jobCards = $('.job_seen_beacon, .result, .tapItem, [id^="job_"]');
            jobCards.each((_, el) => {
        ...
        if (selectedSources.includes("SwissDevJobs")) {
          ...
          try {
            ...
            $('.job-list-item').each((_, el) => {
              ...
              if (title && urlValue) {
                const normUrl = normalizeJobUrl(urlValue);
                if (!jobsMap.has(normUrl)) {
      ...
      console.log(`[Scraping] Completed. Total unique jobs found: ${allJobs.length}`);
